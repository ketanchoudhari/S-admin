...
@Component({
  selector: 'app-casino-productlist',
  templateUrl: './casino-productlist.component.html',
  styleUrls: ['./casino-productlist.component.scss']
})
export class CasinoProductlistComponent implements OnInit {
  productlist: any = [];
  selectedUser: string = '';
  ...
  ActivateCasinoProducts() {
    if (
      ...
      const params = {
        ...
        products: Object.keys(this.teenpattiSelectMap)
  ...
  CasinoProducts() {
    ...
    this.casino.ListCasinoProducts().subscribe((res: any) => {

      if (res && res.errorCode === 0) {
        res.result.forEach(
          (t) => (this.teenpattiSelectMap[t.product] = false)
        );
  ...
  onChange(userid) {
    ...
    this.casino.userCasinoProducts(userid).subscribe((res: any) => {
      // console.log(res)
        ...
        this.productlist.forEach((data) => {
          ...
          this.productlist1.forEach((element) => {
            if (data.product == element.product) {
              this.productlist.forEach(
                (t) => (this.teenpattiSelectMap[element.product] = true)
              );
            }
            else {
              // data.activeStatus=false
