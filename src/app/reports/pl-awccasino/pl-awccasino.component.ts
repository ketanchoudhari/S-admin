...
@Component({
  ...
  styleUrls: ['./pl-awccasino.component.scss']
})
export class PlAwccasinoComponent implements OnInit ,OnDestroy{
    ...
    constructor(
      ...
      private awcCasino:AwcCasinoService
    ) {
    ...
    // }
    getdownlinedata(){
      this.awcCasino.casinoAWCPlReportByDownLine(
    ...
    casinoPlReportByDownLine(){
      ...
      let casinoPlReportByDownLine = this.awcCasino.casinoAWCPlReportByDownLine(
        `${this.fromDate} ${this.fromTime}`,
        `${this.toDate} ${this.toTime}`)
        ...
        .subscribe((res: GenericResponse<any[]>) => {
          ...
          if (res.errorCode === 0) {
            ...
            this.downlineList = PlAwccasinoComponent.addShowDownline(
              res.result[0]
  
            );
            this.downlineListHolder = PlAwccasinoComponent.addShowDownline(
              res.result[0]
            );
    ...
    private static addShowDownline(downline: IDownlinePL[]) {
      return downline.map((downline) => {
        ...
        PlAwccasinoComponent.addShowDownline(downline.downLine);
        return downline;
    ...
    CasinoAWCPnl() {
      ...
      `${this.toDate} ${this.toTime}`,this.p, this.n,(this.selectedUser !== "")?+this.selectedUser:+this.currentUser?.userId).subscribe((res: any) => {
        ...
        res.result.sort((a, b) => a.totalPNL - b.totalPNL);
