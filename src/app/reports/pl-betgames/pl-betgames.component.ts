...
@Component({
  ...
  styleUrls: ['./pl-betgames.component.scss']
})
export class PlBetgamesComponent implements OnInit {
  ...
  // }
  getdownlinedata(){
    this.reportsService.CasinoBGPlReportByDownLine(
      `${this.fromDate} ${this.fromTime}`,
      `${this.toDate} ${this.toTime}`,`${ this.currentUser.userId}`)
      .subscribe((res: any) => {
  ...
  casinoPlReportByDownLine(){
    ...
    let casinoPlReportByDownLine = this.reportsService.CasinoBGPlReportByDownLine(
      `${this.fromDate} ${this.fromTime}`,
      `${this.toDate} ${this.toTime}`,`${ this.currentUser.userId}`)
      ...
      .subscribe((res: GenericResponse<any[]>) => {
        ...
        if (res.errorCode === 0) {
          ...
          this.downlineList = PlBetgamesComponent.addShowDownline(
            res.result[0]

          );
          this.downlineListHolder = PlBetgamesComponent.addShowDownline(
            res.result[0]
          );
  ...
  private static addShowDownline(downline: IDownlinePL[]) {
    return downline.map((downline) => {
      ...
      PlBetgamesComponent.addShowDownline(downline.downLine);
      return downline;
  ...
  slotCasino() {
    this.loadingService.setLoading(true);
    this.reportsService.CasinoBGPnl(  
       `${this.fromDate} ${this.fromTime}`,
