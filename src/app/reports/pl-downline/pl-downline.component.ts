...
// import * as wlrepoortjson from '../../../assets/reports/wlpnlreport.json';
...
export class PlDownlineComponent implements OnInit, OnDestroy {
  ...
  // }
  // getdownlinedata(){
  //   this.reportsService.plReportByDownLine(
  ...
  getDownlinePL() {
    ...
    let plByDownlineSub = this.reportsService.plReportByDownLine(
      `${this.fromDate} ${this.fromTime}`,
      `${this.toDate} ${this.toTime}`)
      ...
      .subscribe((res: GenericResponse<any[]>) => {
        ...
        if (res.errorCode === 0) {
          ...
          this.downlineList = PlDownlineComponent.addShowDownline(
            res.result[0]

          );
          this.downlineListHolder = PlDownlineComponent.addShowDownline(
            res.result[0]
          );
    ...
    this.subSink.add(plByDownlineSub);
  }
  ...
  private static addShowDownline(downline: IDownlinePL[]) {
    return downline.map((downline) => {
      ...
      PlDownlineComponent.addShowDownline(downline.downLine);
      return downline;
