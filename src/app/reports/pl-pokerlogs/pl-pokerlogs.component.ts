...
@Component({
  selector: 'app-pl-pokerlogs',
  templateUrl: './pl-pokerlogs.component.html',
  styleUrls: ['./pl-pokerlogs.component.scss']
})
export class PlPokerlogsComponent implements OnInit, OnDestroy {
  ...
  ReportPokerLog() {
    ...
    let casinoPlReportByDownLine = this.reportsService.ReportPokerLog(
      ...
      .subscribe((res: GenericResponse<any[]>) => {
        ...
        if (res.errorCode === 0) {
          ...
          this.totPNL = 0;
          this.totStake = 0;
          this.downlineList.forEach((user: any) => {
            this.totPNL += user.AmountWon;
            this.totStake += user.AmountBet;
          });
        }
        this.exportFiles();
      });
  ...
  private static addShowDownline(downline: IDownlinePL[]) {
    return downline.map((downline) => {
      ...
      PlPokerlogsComponent.addShowDownline(downline.downLine);
      return downline;
