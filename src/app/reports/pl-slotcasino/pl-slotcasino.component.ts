...
@Component({
  selector: 'app-pl-slotcasino',
  templateUrl: './pl-slotcasino.component.html',
  styleUrls: ['./pl-slotcasino.component.scss']
})
export class PlSlotcasinoComponent implements OnInit {
  ...
  casinoPlReportByDownLine() {
    ...
    let casinoPlReportByDownLine = this.reportsService.casinoSlotPlReportByDownLine(
      ...
      .subscribe((res: GenericResponse<any[]>) => {
        ...
        if (res.errorCode === 0) {
          ...
          this.downlineList = PlSlotcasinoComponent.addShowDownline(
            res.result[0]

          );
          this.downlineListHolder = PlSlotcasinoComponent.addShowDownline(
            res.result[0]
          );
  ...
  private static addShowDownline(downline: IDownlinePL[]) {
    return downline.map((downline) => {
      ...
      PlSlotcasinoComponent.addShowDownline(downline.downLine);
      return downline;
  ...
  slotCasino() {
    this.loadingService.setLoading(true);
    this.reportsService.CasinoSlotPnl(
      `${this.fromDate} ${this.fromTime}`,
