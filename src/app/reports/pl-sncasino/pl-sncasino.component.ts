...
import { SncasinoService } from 'src/app/sncasino/sncasino.service';
...
@Component({
  selector: 'app-pl-sncasino',
  templateUrl: './pl-sncasino.component.html',
  styleUrls: ['./pl-sncasino.component.scss']
})

export class PlSncasinoComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private sncasino: SncasinoService,
  ...
  casinoPlReportByDownLine(){
    ...
    let casinoPlReportByDownLine = this.reportsService.casinoSNPlReportByDownLine(
      ...
      .subscribe((res: GenericResponse<any[]>) => {
        ...
        if (res.errorCode === 0) {
          ...
          this.downlineList = PlSncasinoComponent.addShowDownline(
            res.result[0]

          );
          this.downlineListHolder = PlSncasinoComponent.addShowDownline(
            res.result[0]
          );
  ...
  private static addShowDownline(downline: IDownlinePL[]) {
    return downline.map((downline) => {
      ...
      PlSncasinoComponent.addShowDownline(downline.downLine);
      return downline;
  ...
  SNCasino() {
    this.loadingService.setLoading(true);
    this.sncasino.CasinoSNPnl(  
       `${this.fromDate} ${this.fromTime}`,
    `${this.toDate} ${this.toTime}`,this.p, this.n,(this.selectedUser !== "")?+this.selectedUser:this.currentUser?.userId).subscribe((res: any) => {
